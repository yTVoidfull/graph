...
public class DijkstraSearch<N> {
    ...
    public DijkstraSearch(N start, N end, WeightedTraversableGraph<N> graph) {
        this.start = start;
        this.end = end;
        this.graph = graph;
    ...
    private void checkNodesToBeVisited(N node, List<N> neighbors, double currentWeight) {
        for (N n : neighbors) {
            if (!visited.contains(n)) {
                Double newWeight = currentWeight + graph.getEdgeWeight(node, n);
